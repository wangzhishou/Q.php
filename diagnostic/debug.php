...
function getImg(){
    if(isset($_GET['diagnostic_pic']) && file_exists(Q::conf()->BASE_PATH . 'diagnostic/assets/'.$_GET['diagnostic_pic'])){
        header('Content-Type: image/png');
        header('Content-Length: '.filesize(Q::conf()->BASE_PATH . 'diagnostic/assets/'.$_GET['diagnostic_pic']));
        ob_clean();
        readfile(Q::conf()->BASE_PATH . 'diagnostic/assets/'.$_GET['diagnostic_pic']);
        exit;
    }
}
...
function setExceptionHandler($e){
	...
	$err = printVar($e);
	global $errTrace;
	$errTrace = TextHelper::highlightPHP($err);
	if(preg_match_all('/\[file\] \=\> (.+)\n/', $err, $matches)){
		...
		if($e instanceof PDOException){
			foreach($last as $k=>$l){
				if(strpos(str_replace('\\','/',$l), str_replace('\\','/',Q::conf()->SITE_PATH))===0){
					$lastfile = $l;
					break;
				}
			}
...
function setErrorHandler($errno, $errstr, $errfile, $errline, $errcontext=null){
	...
	//require
	if($errno==2 && strpos($errstr, 'require_once(')===0 && strpos(str_replace('\\','/',$errfile), str_replace('\\','/',Q::conf()->BASE_PATH).'.php')===0){
		$dmsg = debug_backtrace();
		foreach($dmsg as $k=>$a){
			//if is user file, and must be a load method
			if(strpos(str_replace('\\','/',$a['file']), str_replace('\\','/',Q::conf()->SITE_PATH))===0
    ...
    else if($errno==2 && strpos(str_replace('\\','/',$errfile), str_replace('\\','/',Q::conf()->BASE_PATH).'view/View.php')===0){
		$dmsg = debug_backtrace();
		foreach($dmsg as $k=>$a){
            if(strpos(str_replace('\\','/',$a['file']), str_replace('\\','/',Q::conf()->SITE_PATH))===0
	...
	if(strpos(str_replace('\\','/',$errfile), str_replace('\\','/',Q::conf()->BASE_PATH))===0){
		$dmsg = debug_backtrace();
		$last = array_pop($dmsg);
		if(isset($last['file'])){
//			$errfile = $last['file'];
//			$errline = $last['line'];
    ...
    $script = TextHelper::highlightPHP($script);
	...
	if(!empty($script)){
        ...
        if(strpos(str_replace('\\','/',$errfile), str_replace('\\','/',Q::conf()->SITE_PATH.Q::conf()->PROTECTED_FOLDER).'view')===0){
            $pre = '<pre>';
            $Xpre = '</pre>';
        }
    ...
    $imgloader = Q::conf()->SUBFOLDER . 'index.php?diagnostic_pic=';

	if (ob_get_level() !== 0) {
		ob_clean();
	}
	$confData = traceVar(Q::conf());
    ...
    echo "<html><head><title>PHP Diagnostics - $errstr</title>";
    ...
    echo "<span id=\"logo\" onclick=\"javascript:goVisit();\">PHP</span><h1>$errstr</h1>";
