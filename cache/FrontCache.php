...
/**
 * FrontCache provides frontend caching utilities.
 * <p>It can be retrieved with the shorhand Q::cache('front'). The frontend cache supports full page and partial page caching mechanism.
 ...
 * <code>
 * $cacheOK = Q::cache('front')->testPart('latestuser');
 ...
 * <code>
 * //Use it in index.php
 * $check = Q::cache('front')->getOnly('/blog', 3600, true);
 *
 * //Start recording and cache the page.
 * if($check)
 *     Q::cache('front')->start();
 ...
 *
 * @author Leng Sheng Hong <darkredz@gmail.com>
 * @version $Id: FrontCache.php 1000 2009-08-11 18:28:42
 * @package .cache
 * @since 1.1
 */
class FrontCache{
	...
	public function __construct($path='') {
		if ( $path=='' ) {
			if(isset(Q::conf()->CACHE_PATH))
				$this->_directory = Q::conf()->CACHE_PATH . 'frontend/';
			else
				$this->_directory = Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER . 'cache/frontend/';
		}else{
	 ...
	 * <code>Q::cache('front')->getOnly('/blog', 3600);
	 * Q::cache('front')->getOnly(array('/allurl','/gen_site', '/blog'), 3600);
	 * </code>
	 *
	 ...
	 * Q::cache('front')->getOnly('/blog', 3600, true);
	 * </code>
	 *
	 ...
	 * Q::cache('front')->getOnly(array('/allurl','/gen_site', '/blog'), 3600, array('/blog', '/gen_site'));
	 * </code>
	 *
	 ...
	 * Q::cache('front')->getExcept('/blog', 3600);
	 * Q::cache('front')->getExcept(array('/allurl','/gen_site', '/blog'), 3600);
	 * </code>
	 *
	 ...
	 * Q::cache('front')->getExcept('/blog', 3600, true);
	 * </code>
	 *
	 ...
	 * // Only /blog & /gen_site will be check recursively.
	 * Q::cache('front')->getExcept(array('/allurl','/gen_site', '/blog'), 3600, array('/blog', '/gen_site'));
	 * </code>
	 *
	 * @param string $url URL to be output if cache exists/not expired.
	 * @param int $secondsCache Duration till the cache expired
	...
	* <code>
	*  $userListContent = "";
	*  if (($userListContent = Q::cache('apc')->get('userList')) == false) {
	*   Q::cache('front')->startApc("userList");  ?>SOME HTML GOES HERE
	*  <?php
	*   Q::cache('front')->endApc();
	*  } else {
	...
	*/
	...
	public function endApc() {
		Q::cache('apc')->set($this->_cachefile, ob_get_contents(), 60);
		ob_end_flush();
	}
	...
	*/
	...
	public function getPartApc($id, $secondsCache=3600) {
		if (false !== ($data = Q::cache("apc")->get($id))) {
			echo $data;
			return true;
		}
		return false;
