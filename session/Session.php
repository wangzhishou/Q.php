...
* Call it from:
* <code>$session = Q::session("mysite");</code>
...
class Session {
	...
	/**
	* Namespace - Name of  session namespace
	*
	* @var string
	*/
	...
	*/
	public function __construct($namespace = 'Default', $sessionId = null) {
		...
		// should not be empty
		if ($namespace === '') {
			throw new SessionException('Namespace cant be empty string.');
		}
		// should not start with underscore
		if ($namespace[0] == "_") {
			throw new SessionException('You cannot start session with underscore.');
		}
		// should not start with numbers
		if (preg_match('#(^[0-9])#i', $namespace[0])) {
			throw new SessionException('Session should not start with numbers.');
		}
	...
	*/
	public function __set($name, $value) {
		if ($name === "")
			throw new SessionException("Keyname should not be empty string!");
		if (!$this->_sessionStarted)
	...
	*/
	public function namespaceUnset($name = null) {
		if (!$this->_sessionStarted) {
			throw new SessionException("Session not started, use Session::start()");
		}
		if (empty($name)) {
	...
	*/
	public function setId($id) {
		if (isset($_SESSION))
			throw new SessionException("Session is already started, id must be set before.");
		if (!is_string($id) || $id === '')
			throw new SessionException("Session id must be string and cannot be empty.");
		if (headers_sent($filename, $linenum))
			throw new SessionException("Headers already sent, output started at " . $filename . " on line " . $linenum);
		session_id($id);
	}
	...
	*/
	public function &get($name) {
		...
		if ($name == '')
			throw new SessionException("Name should not be empty");
	...
	*/
	...
	/**
	* Check if Session variable is stored
	* @return bool
	*/
