 ...
 *
 * <p>View is a compiling template engine. It parses the HTML templates and convert them into PHP scripts which will then be included and rendered.
 * In production mode, View will not processed the template but include the compiled PHP file instead. Otherwise,
 ...
 *
 * <p>Loops, variables, function calls with parameters, include files are supported in View. View only allows a template to use functions defined in <i>TemplateTag.php</i>.
 ...
 * <!-- endcache -->
 * </code>
 * <p>View can be called from the controller: </p>
 * <code>
 * class SampleController extends Controller{
 *      public viewme(){
 *          $data['myname'] = 'My name is ';
 ...
 * </code>
 *
 * <p>You can use <b>native PHP</b> as view templates in PHP. Use View::renderc() instead of render.</p>
...
class View {
    ...
    public function renderc($file, $data=NULL, $controller=NULL, $includeTagClass=TRUE){
        ...
        if($includeTagClass===TRUE)
            $this->loadTagClass();
        include Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER . "/viewc/$file.php";
    }
    ...
    public function render($file, $data=NULL, $process=NULL, $forceCompile=false){
        ...
        if(isset(Q::conf()->TEMPLATE_COMPILE_ALWAYS) && Q::conf()->TEMPLATE_COMPILE_ALWAYS==true){
        ...
        if($process!=true){
            ...
            $cfilename = Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER . "viewc/$file.php";
            $vfilename = Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER . "view/$file.html";
    ...
    public function renderLayout($layoutName, $viewFile, $data=NULL, $process=NULL, $forceCompile=false) {
        ...
        if($process!=true){
            //includes user defined template tags for template use
            $this->loadTagClass();
            include Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER . "viewc/$compiledViewFile.php";
        }
        else{
            $lfilename = Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER . "layout/$layoutName.html";
            $vfilename = Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER . "view/$viewFile.html";
            $cfilename = Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER . "viewc/$compiledViewFile.php";
     ...
     * stores the resulting view file to then be processed as normal by View::compile
    ...
    public function loadTagClass(){
        ...
        if( !isset($this->tagClassName) ){
            if( !isset(Q::conf()->PROTECTED_FOLDER_ORI) ){
                $tagFile = Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER . 'plugin/TemplateTag.php';
                $tagcls = 'TemplateTag';
            }else{
                $tagcls = explode('/', Q::conf()->PROTECTED_FOLDER);
                $tagcls = ucfirst($tagcls[sizeof($tagcls)-2]) . 'Tag';
                $tagFile = Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER . 'plugin/' . $tagcls .'.php';
            }
        }else{
            //load the main app's TemplateTag if module is '/'
            if($this->tagModuleName=='/'){
                $tagFile = Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER_ORI . 'plugin/'. $this->tagClassName .'.php';
            }
            else if($this->tagModuleName===Null){
                $tagFile = Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER . 'plugin/'. $this->tagClassName .'.php';
            }
            else{
                if(isset(Q::conf()->PROTECTED_FOLDER_ORI))
                    $tagFile = Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER_ORI .'module/'. $this->tagModuleName . '/plugin/'. $this->tagClassName .'.php';
                else
                    $tagFile = Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER .'module/'. $this->tagModuleName . '/plugin/'. $this->tagClassName .'.php';
            }
    ...
    private function setTags(){
		...
		if ($tagcls === false) {
			...
			if(!empty($tagMethod)){
				if( !empty(Q::conf()->TEMPLATE_GLOBAL_TAGS) )
					$template_tags = array_merge(Q::conf()->TEMPLATE_GLOBAL_TAGS, $tagMethod);
				else
			...
			else if( !empty(Q::conf()->TEMPLATE_GLOBAL_TAGS) ){
				$template_tags = Q::conf()->TEMPLATE_GLOBAL_TAGS;
			}
			else{
				$template_tags = array();
			}
        ...
        Q::conf()->add('TEMPLATE_TAGS', $template_tags);
        return $template_tags;
    }

    /**
     * Processes a string containing PHP Template tags and replaces them with the relevant PHP code required
    ...
    private function compileTags($str) {
        ...
        if($this->tags===NULL){
            if(!isset(Q::conf()->TEMPLATE_TAGS)){
                $this->tags = $this->setTags();
            }else{
                $this->tags = Q::conf()->TEMPLATE_TAGS;
            }
        }

        if( isset(Q::conf()->TEMPLATE_ALLOW_PHP) ){
            if( Q::conf()->TEMPLATE_ALLOW_PHP === False ){
        ...
        //remove comments
        if(!isset(Q::conf()->TEMPLATE_SHOW_COMMENT) || Q::conf()->TEMPLATE_SHOW_COMMENT!=true){
    ...
    private function parseFunc($matches){
        ...
        if(!empty($matches[1])){
            ...
            //skip checking static method usage: TemplateTag::go(), Q::conf()
            if(stripos($funcname, $this->tags['_class'] . '::')===False && stripos($funcname, '')===False){
    ...
    private function convertCache($matches){
        ...
        if(sizeof($data)==2){
            ...
            return "<?php if (!Q::cache('front')->getPart({$data[0]}, {$data[1]})): ?>\n<?php Q::cache('front')->start({$data[0]}); ?>";
        }else{
            return "<?php if (!Q::cache('front')->getPart({$data[0]})): ?>\n<?php Q::cache('front')->start({$data[0]}); ?>";
        }
    }
    ...
    private function convertInclude($matches){
        ...
        if(substr($file, 0,1)=='/'){
            $file = substr($file, 1);
            $cfilename = str_replace('\\', '/', Q::conf()->SITE_PATH) . Q::conf()->PROTECTED_FOLDER . "viewc/$file.php";
            $vfilename = str_replace('\\', '/', Q::conf()->SITE_PATH) . Q::conf()->PROTECTED_FOLDER . "view/$file.html";
        }
        else{
        ...
        return '<?php include Q::conf()->SITE_PATH .  Q::conf()->PROTECTED_FOLDER . "viewc/'.$file.'.php"; ?>';
    }
