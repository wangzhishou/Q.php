...
class ViewBasic {
	...
	public function __construct() {
		Q::loadHelper('File');
		$this->fileManager = new File(0777);
		$this->defaultRootViewPath = Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER . 'view/';
		$this->rootViewPath = Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER . 'view/';
		$this->rootCompiledPath = Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER . 'viewc/';
		if (self::$uniqueId == 0) {
			self::$uniqueId = time();
		}
	}
	...
	public function enableSafeVariableAccess($enable = true, $defaultValue = null) {
		...
		ViewBasic::$safeVariableResult = $defaultValue;
	}
	...
	protected function block_for($params) {
		...
		$forStmt = '';
		$postForStatement = '';
		$namespace = isset(Q::conf()->APP_NAMESPACE) ? Q::conf()->APP_NAMESPACE : '';
	...
	protected function block_cache($params) {
		if ( preg_match('/\(([^\t\r\n]+?)(([,][ ]*)([0-9]+[ ]*))?[ ]*\)[ ]*$/', $params, $matches)) {
			...
			if (count($matches) == 5) {
				return "<?php if (!Q::cache('front')->getPart({$matches[1]}, {$matches[4]})): ?>\n<?php Q::cache('front')->start({$matches[1]}); ?>";
			} else {
				return "<?php if (!Q::cache('front')->getPart({$matches[1]})): ?>\n<?php Q::cache('front')->start({$matches[1]}); ?>";
			}
		}
	...
	protected function block_include($params) {
		...
		// Using dynamic variable for the include
			...
			$content  = "<?php\n";
			$content .= "\tif({$filePath} != ViewBasic::\$safeVariableResult){\n";
	...
	protected function extractDataPath($str, $ignoreSafe = false) {
		...
		if (!$ignoreSafe && $this->useSafeVariableAccess) {
			return 'ViewBasic::is_set_or(' . $result . ')';
		} else {
			return $result;
		}
	}
	...
	public function getUniqueVarId() {
		return '$_view_basic_' . self::$uniqueId++;
	}

	public static function is_set_or(&$var) {
		return (isset($var)) ? $var : ViewBasic::$safeVariableResult;
	}
