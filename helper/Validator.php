 ...
 *
 * <p>Validator can be used for form and Model data validation before saving/inserting/deleting a data.</p>
 *
 * <p>To use Validator, you have to create an instance of it and defined the validation rules.
 ...
 *
 * <p>To validate the data, create an instance of Validator and call validate() in your Controller/Model.</p>
 * <code>
 * $v = new Validator();
 *
 ...
 * //$data = array('username'=>'php', 'pwd'=>'12345');
 ...
 * If your Models are extending Model or SmartModel, you can validate the data by calling Model::validate()</p>
 * <code>
 * $f = new Food;
...
class Validator {
    ...
    public function validate($data=null, $rules=null){
        ...
        if(is_string($rules)){
            $rules = include(Q::conf()->SITE_PATH .  Q::conf()->PROTECTED_FOLDER . 'config/forms/'.$rules.'.php');
        }

        $optErrorRemove = array();

        foreach($data as $dk=>$dv){
            if($this->requireMode == Validator::REQ_MODE_NULL_EMPTY && ($dv === null || $dv === '') ||
			   $this->requireMode == Validator::REQ_MODE_NULL_ONLY  && $dv === null){
                unset($data[$dk]);
			}
        }
        ...
        foreach($data as $k=>$v){
            ...
            foreach($cRule as $v2){
                if(is_array($v2)){
					...
					$vIsEmpty = ($this->requireMode == Validator::REQ_MODE_NULL_EMPTY && ($v === null || $v === '') ||
								 $this->requireMode == Validator::REQ_MODE_NULL_ONLY  && $v === null) ? true : false;
                    ...
                    if($err = call_user_func_array(array(&$this, 'test'.$v2[0]), $vv) ){
                        if($this->checkMode==Validator::CHECK_ALL)
                            $errors[$k][$v2[0]] = $err;
                        else if($this->checkMode==Validator::CHECK_SKIP && !$vIsEmpty && $v2[0]!='optional'){
                            return $err;
                        }else if($this->checkMode==Validator::CHECK_ALL_ONE)
                            $errors[$k] = $err;
                    }
                    ...
                    if(sizeof($cRule)>1){
                        ...
                        if($err = call_user_func_array(array(&$this, 'test'.$cRule[0]), $vv) ){
                            if($this->checkMode==Validator::CHECK_ALL || $this->checkMode==Validator::CHECK_ALL_ONE)
                                $errors[$k] = $err;
                            else if($this->checkMode==Validator::CHECK_SKIP){
                                return $err;
                            }
     ...
     * @param type $displayMethod Default error message display method. use: Validator::REQ_MSG_UNDERSCORE_TO_SPACE, Validator::REQ_MSG_CAMELCASE_TO_SPACE, Validator::REQ_MSG_THIS, Validator::REQ_MSG_FIELDNAME
     * @param type $suffix suffix for the error message. Default is ' field is required'
     */
    public function setRequiredFieldDefaults( $displayMethod = Validator::REQ_MSG_UNDERSCORE_TO_SPACE, $suffix = ' field is required'){
    ...
    public function getRequiredFieldDefaultMsg($fieldname){
        if($this->requiredMsgDefaultMethod==Validator::REQ_MSG_UNDERSCORE_TO_SPACE)
            return ucfirst(str_replace('_', ' ', $fieldname)) . $this->requiredMsgDefaultSuffix;
        
        if($this->requiredMsgDefaultMethod==Validator::REQ_MSG_THIS)
            return 'This ' . $this->requiredMsgDefaultSuffix;        
        
        if($this->requiredMsgDefaultMethod==Validator::REQ_MSG_CAMELCASE_TO_SPACE)
            return ucfirst(strtolower(preg_replace('/([A-Z])/', ' $1', $fieldname))) . $this->requiredMsgDefaultSuffix;
        
        if($this->requiredMsgDefaultMethod==Validator::REQ_MSG_FIELDNAME)
            return $fieldname . $this->requiredMsgDefaultSuffix;
    }

    public function testOptional($value){}
    public function testRequired($value, $msg){
		if ($this->requireMode == Validator::REQ_MODE_NULL_EMPTY && ($value === null || $value === '') ||
			$this->requireMode == Validator::REQ_MODE_NULL_ONLY  && $value === null) {
    ...
    */
    public function testDbExist($value, $table, $field, $msg=null) {
        $result = Q::db()->fetchRow("SELECT COUNT($field) AS count FROM " . $table . ' WHERE '.$field.' = ? LIMIT 1', array($value));
