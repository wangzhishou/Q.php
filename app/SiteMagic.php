...
/**
 * SiteMagic provides useful tools in development.
 *
 * <p>If you have your routes defined, call SiteMagic::buildSite() and
...
class SiteMagic{
    ...
    /**
     * Display some info as a welcome page for PHP
     */
    public static function displayHome(){
        echo '<div style="font-family:\'Courier New\', Courier, monospace;"><h1>It Works!</h1>';
        echo '<h3>What now?</h3><p><a href="'. Q::conf()->APP_URL .'tools/sitemap.html">Generate Sitemap</a> | <a href="'. Q::conf()->APP_URL .'index.php/gen_site">Generate Controllers</a> | <a href="'. Q::conf()->APP_URL .'index.php/gen_model">Generate Models</a> | <a href="'. Q::conf()->APP_URL .'tools/logviewer.html">View Logs</a> | <a href="'. Q::conf()->APP_URL .'index.php/allurl">View All URLs</a></p>';
    ...
    public static function showDebug($filename){
        $path = isset(Q::conf()->LOG_PATH) ? Q::conf()->LOG_PATH : Q::conf()->SITE_PATH;
            ...
            foreach($r as $rname=>$value){
                if($rname=='root' || $rname=='catchall'){
                    foreach($value as $rname2=>$value2){
                        ...
                        $data[$n++ ." $req"] = '<a href="'.Q::conf()->APP_URL.$rname_strip.'">'.$rname2.'</a>';
                    }
                }else{
                    $rname_strip = 'index.php'.$rname;
                    $data[$n++ ." $req"] = '<a href="'.Q::conf()->APP_URL.$rname_strip.'">'.$rname.'</a>';
                }
            }
        }
        echo '<pre>';
    ...
    public static function buildSitemap($replace=false){
        if(!isset($_POST['routes']) || empty($_POST['routes'])){
            ...
            $handle = fopen(Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER . 'config/'.$replacename.'.conf.php', 'w+');
    ...
    public static function buildSite(){
        include Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER . 'config/routes.conf.php';
        $controllers = array();
        foreach($route as $req=>$r){
            foreach($r as $rname=>$value){
        ...
        echo "<html><head><title>PHP Site Generator </title></head><body bgcolor=\"#2e3436\">";
        $total = 0;
        ...
        foreach($controllers as $cname=>$methods){
            ...
            $filestr .= "\n\n}\n?>";
            if(file_exists(Q::conf()->SITE_PATH. Q::conf()->PROTECTED_FOLDER . 'controller/'.$cname.'.php')){
                ...
                $total++;
                $handle = fopen(Q::conf()->SITE_PATH. Q::conf()->PROTECTED_FOLDER . 'controller/'.$cname.'.php', 'w+');
