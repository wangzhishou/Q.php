...
/**
 * CliController is the controller for CLI application should be use with Q::app('CliApp')->run();
 * @author Richard Myers <richard.myers@hotmail.co.uk>
 * @package .controller
 * @since 1.4 
 */
Q::loadCore('app/CliApp');
class CliController {
	...
	protected function displayMenu($question, $options, $useNumericalValueSelection = false,
		...
		do {
			...
			$choice = trim(fgets(CliApp::$streamStdIn));
	...
	protected function displayMultiSelectMenu($question, $options,
		...
		do {
			...
			$choices = explode(',', trim(fgets(Q::app()->streamStdIn)));
	...
	public function runCommandLineTask($command, $input='') {
		CliApp::runCommandLineTask($command, $input);
	}
