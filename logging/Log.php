...
class Log{
    ...
    /**
     * Turn on/off debug mode for Log
     ...
     * @param string $msg
     * @param int $level default Log::TRACE
	...
	public function beginDbProfile($token, $category='database'){
		...
		$this->_logs[] = $log;
		$this->_profiles[$token] = $log;
        $query_index = Q::db()->getQueryCount();
     ...
     * @param int $level Filter by a log level, eg. Log::INFO
     * @param string $category Filter by a category name
     * @param bool $xml Whether to write as plain text or XML file.
     */
    ...
    protected function writeToFile($data, $filename){
        ...
        if($data!=NULL){
            $mode = 'a+';
            if(isset(Q::conf()->LOG_PATH))
                $filename = Q::conf()->LOG_PATH . $filename;
