...
abstract class ManageDb {
	...
	/**
	 * A mapping of  generic datatypes to RDBMS native datatypes for columns
	...
	protected $colTypeMapping = array (
		ManageDb::COL_TYPE_BOOL		=> null,
    	ManageDb::COL_TYPE_SMALLINT	=> null,
    	ManageDb::COL_TYPE_INT		=> null,
    	ManageDb::COL_TYPE_BIGINT	=> null,
    	ManageDb::COL_TYPE_DECIMAL	=> null,
    	ManageDb::COL_TYPE_FLOAT		=> null,
    	ManageDb::COL_TYPE_CHAR		=> null,
    	ManageDb::COL_TYPE_VARCHAR	=> null,
    	ManageDb::COL_TYPE_CLOB		=> null,
    	ManageDb::COL_TYPE_DATE		=> null,
    	ManageDb::COL_TYPE_TIME		=> null,
    	ManageDb::COL_TYPE_TIMESTAMP	=> null,
	);
    ...
    public function connect(){
        ...
        try {
            ...
            throw new ManageDbException('Failed to open the DB connection', $ex->getMessage());
        }
    }
	 ...
	 * }}
	 *
	 * The field types are defined in the _DB_COL_TYPE_* definitions which are defined in the DbManager.php file
	 *
	...
	public function createTable($table, $cols, $options=null) {
		...
		try {
			...
			throw new ManageDbException("Error Creating Table : $table", $ex->getMessage());
		}
	}
	...
	protected function _sqlCreateTable($table, $columnDefinitions, $options=null) {
		...
		foreach($columnDefinitions as $name => $attributes) {
			...
			} catch (DbAdminException $ex) {
				$errors[$name] = $ex->getMessage();
			}
		}
		if ($errors) {
			throw new ManageDbException("Could not create table", implode("\n", $errors));
		}
	...
	public function dropTable($table) {
		...
		try {
			...
			throw new ManageDbException("Error Dropping Table : $table", $ex->getMessage());
		}
	}
	...
	public function addColumn($table, $name, array $attributes) {
		...
		try {
			...
			throw new ManageDbException("Error Adding Column ($name) to Table ($table)", $ex->getMessage());
		}
	}
	...
	public function dropColumn($table, $name) {
		...
		try {
			...
			throw new ManageDbException("Error Dropping Column ($name) from Table ($table)", $ex->getMessage());
		}
	}
	...
	protected function buildColumnDefinition($name, $attributes) {
        ...
        if (array_key_exists($type, $this->colTypeMapping) == false) {
            throw new ManageDbException('Unrecognised column type specified', "$type is not a recognised database type. Please check DbManager::colTypeMapping for supported column types");
        }

        // get the column type defenition
        switch ($type) {

        	case ManageDb::COL_TYPE_CHAR:
        	case ManageDb::COL_TYPE_VARCHAR:
	            if ($size < 1 || $size > 255) {
	                throw new ManageDbException("Invalid Column Size ($size) for $name", "char and varchar require a column size between 1 and 255");
	            } else {
					$columnDefinition = $this->colTypeMapping[$type] . "($size)";
				}
	            break;

	        case ManageDb::COL_TYPE_DECIMAL:
	            if ($size < 1 || $size > 255) {
	                throw new ManageDbException("Invalid Column Size ($size) for $name", "decimal requires size to be between 1 and 255");
	            } elseif ($scope < 0 || $scope > $size) {
	                throw new ManageDbException("Invalid Column Scope ($scope) for $name", "Scope must be between 0 and the Column Size ($size)");
	            } else {
	...
	protected function checkIdentifier($type, $name) {
        ...
        $len = strlen($name);
        if ($len < 1 || $len > 30) {
            throw new ManageDbException("Invalid Identifer for $type : $name", "Identifiers must be between 1 and 30 characters long");
        }
        ...
        if (preg_match($regex, $name) == false) {
            throw new ManageDbException("Invalid Identifer for $type : $name", "Identifer must start with a letter a-z and contain only letters, numbers and underscores");
        }

        // must not have two or more underscores in a row
        if (strpos($name, '__') !== false) {
            throw new ManageDbException("Invalid Identifer for $type : $name", "Identifier can not contain 2 or more underscores consecutavily");
        }
	}
}


class ManageDbException extends Exception {
	/**
	 * An exception thrown by the DbAdmin
