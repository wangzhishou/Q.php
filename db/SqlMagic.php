...
/**
 * SqlMagic is a fast ORM tool which handles relational data.
 *
 * <p>It generates SQL query on the fly and escaping/quoting record values to prevent sql injection.
 * You can do CRUD operations in an easier way with SqlMagic instead of writing queries manually.</p>
 *
 * <p>Database models does not need to extend any classes with SqlMagic. Database table relationships are defined in a
 * centralized way in <i>SITE_PATH/protected/config/db.conf.php</i> instead of defining them in each Model class.</p>
 *
 * <p>Unlike most active records, SqlMagic class does not use any php Magic methods in order to gain higher performance.</p>
 *
 ...
 *
 * <p>SqlMagic is more like a blend between Active record and DataMapper design pattern.</p>
 *
 * <p>Creating an instance of an SqlMagic class does not immediately connect to the RDBMS server.
...
class SqlMagic {
    ...
    public function loadModel($className, $createObj=false){
        ...
        if(empty($this->modelPath)===true) {
            return Q::loadModel($className, $createObj);
        }
        else {
    ...
    */
    ...
    /**
     * Returns the underlying PDO object used in SqlMagic
     * @return PDO
     */
...
//														echo "<h3>$k2 = $v2</h3>";
     ...
     * Example:
     * <code>
     * Q::db()->relateMany('Food',array('Recipe','Article','FoodType'))
     * </code>
     *
     * @param mixed $model The model class name or object to be select.
     ...
     * Example:
     * <code>
     * Q::db()->relateExpand('Recipe', array('Food','Article'))
     * </code>
     *
     * @param mixed $model The model class name or object to be select.
     * @param array $rmodel The related models class names.
