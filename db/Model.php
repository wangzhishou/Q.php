...
/**
 * Model is the base class for Model classes in PHP that provides useful querying methods.
 *
 * <p>The model classes can extend Model for more powerful ORM features which enable you to write shorter codes.
 ...
 *
 * @author Leng Sheng Hong <darkredz@gmail.com>
 * @version $Id: Model.php 1000 2009-08-27 10:28:56
 * @package .db
 * @since 1.2
 */
class Model{
    ...
    public function relate($rmodel, $opt=null){
        return Q::db()->relate($this, $rmodel, $opt);
    }
    ...
    public static function _relate($model, $rmodel, $opt=null){
        ...
        return Q::db()->relate($model, $rmodel, $opt);
    }
    ...
    public static function _insertAttributes($model, $data){
        return Q::db()->insert_attributes($model, $data);
    }
    
    /**
     * Use Model::_insertAttributes() instead.
     * @deprecated deprecated since version 1.3
     */
    ...
    public function limit($limit=1, $asc='', $desc='', $options=null){
        if($asc!='' || $desc!='' || $options!==null){
            ...
            //print_r($options);
            return Q::db()->find($this, $options);
        }
        return Q::db()->find($this, array('limit'=>$limit));
    }
    ...
    public function getOne($options=null){
        ...
        return Q::db()->find($this, array('limit'=>1));
    }
    ...
    public static function _limit($limit=1, $asc='', $desc='', $options=null){
        if($asc!='' || $desc!='' || $options!==null){
            ...
            return Q::db()->find(self::$className, $options);
        }
        return Q::db()->find(self::$className, array('limit'=>$limit));
    }
    ...
    public static function _getOne($model=null, $options=null){
        ...
        if($options!==null){
            $options['limit'] = 1;
            return Q::db()->find($model, $options);
        }
        return Q::db()->find($model, array('limit'=>1));
    }
    ...
    public function __call($name, $args){
        ...
        if(strpos($name, 'get')===0){
            ...
            if(is_string($field)){
                ...
                if(sizeof($args)>1){
                    if(isset($first))
                        $args[1]['limit'] = 1;
                    return Q::db()->find($obj, $args[1]);
                }

                if(isset($first)){
                    return Q::db()->find($obj, $first);
                }
                return Q::db()->find($obj);
            }
            else{
                $i=0;
                foreach($field as $f){
        ...
        //$food->relateFoodType_first();    this adds the 'limit'=>'first' to the Options
            ...
            if(sizeof($args)===0){
                return Q::db()->relate( $this, $relatedClass);
            }
            else if(sizeof($args)===1){
                if(is_array($args[0])){
                    return Q::db()->relate( $this, $relatedClass, $args[0]);
                }else{
                    if(isset($first)){
                        return Q::db()->relate( $args[0], $relatedClass, $first);
                    }
                    return Q::db()->relate( $args[0], $relatedClass);
                }
            }else{
                return Q::db()->relate( $args[0], $relatedClass, $args[1]);
            }
        }
    }
    ...
    public static function __callStatic($name, $args){
        ...
        //$food->relateFoodType_first();    this adds the 'limit'=>'first' to the Options
            ...
            if(sizeof($args)===0){
                return Q::db()->relate( self::$className, $relatedClass);
            }
            else if(sizeof($args)===1){
                if(is_array($args[0])){
                    return Q::db()->relate( self::$className, $relatedClass, $args[0]);
                }else{
                    if(isset($first)){
                        return Q::db()->relate( $args[0], $relatedClass, $first);
                    }
