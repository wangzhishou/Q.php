...
class Translator {
	...
	* <code>$translator = Q::translator('Gettext', $this->_basePath . 'languages/en/LC_MESSAGES/en.mo', array());</code>
	*
	* Creating class instance for Csv
	* <code>$translator = Q::translator('Csv', $this->_basePath . 'languages/en/LC_MESSAGES/main.csv', array('delimiter' => ';',
	*																									   'enclosure' => '"'));</code>
	...
	*/
	...
	public function __construct($adapter, $data, $options=array()) {
		...
		if (!in_array($adapter, $this->_supportedAdapters)) {
			throw new TranslatorException($adapter . " is not supported by Translator, supported types are: " . implode(', ', $this->_supportedAdapters));
		}
		$this->_setAdapter($adapter);
		if ($data == null) {
			throw new TranslatorException("Data not set!");
		}
		$this->_options = $options;
		// set cache
		if (isset($this->_options['cache'])) {
			if (!in_array($this->_options['cache'], $this->_supportedCacheTypes)) {
				throw new TranslatorException($this->_options['cache'] . " is unsuported cache type, supported cache types are: " . implode(', ', $this->_supportedCacheTypes));
			} else {
				switch ($this->_options['cache']) {
					case "apc":
						$this->_cache = Q::cache('apc');
						break;
					case "php":
						$this->_cache = Q::cache('php');
						break;
					case "xcache":
						$this->_cache = Q::cache('xcache');
						break;
					case "eaccelerator":
						$this->_cache = Q::cache('eaccelerator');
						break;
					case "file":
						if (isset($this->_options['path'])) {
							$this->_cache = new FileCache($this->_options['path']);
						} else {
							throw new TranslatorException("Cache directory is not set, please add path in options.");
						}
						break;
				}
	...
	*/
	...
	public function translate($messageId, $params=array()) {
		if ($this->_adapter == null) throw new TranslatorException("Adapter not set!");
		$translatedMessage = "";
	...
	private function _loadData($data) {
		...
		switch ($this->_adapter) {
			...
			case 'csv':
				...
				if (!$this->_file) {
					throw new TranslatorException('Error opening translation file "' . $filename . '".');
				}
			...
			case 'gettext':
				...
				if (strtolower(substr(dechex($input[1]), -8)) == "950412de") {
					...
					throw new TranslatorException('"' . $filename . '" is not a valid gettext file!');
				}
