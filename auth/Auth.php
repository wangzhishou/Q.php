 ...
 * @version $Id: Auth.php 2009-10-06 15:10:12
 * @package .auth
 * @since 1.3
 *
 */
...
class Auth {
    ...
    /**
     * Constructor - returns an instance object of Auth
     */
    ...
    public function start() {
        $this->appSession = Q::session($this->getApplicationName());
        $this->validate();
    }
    ...
    public function setApplicationName($appName) {
        if (!isset ($appName))
            throw new AuthException("Application name cannot be empty");
        $this->appName = $appName;
    }
    public function getApplicationName() {
        if (!isset ($this->appName))
            throw new AuthException("Application name not defined");
        return $this->appName;
    }
    public function setSalt($salt) {
        if (!isset ($salt))
            throw new AuthException("Salt cannot be empty");
        $this->salt = $salt;
    }
    public function getSalt() {
        if (!isset ($this->salt))
            throw new AuthException("Salt not defined");
        return $this->salt;
    }
    public function setSecurityLevel($securityLevel) {
        if (!isset ($securityLevel))
            throw new AuthException("Security level cannot be empty");
        $this->securityLevel = $securityLevel;
    }
    public function getSecurityLevel() {
        if (!isset ($this->securityLevel))
            throw new AuthException("Security level not defined");
        return $this->securityLevel;
    }
    public function setSessionExpire($sessionExpire) {
        if (!isset ($sessionExpire))
            throw new AuthException("Session expire cannot be empty");
        $this->authSessionExpire = $sessionExpire;
    }
    public function getSessionExpire() {
        if (!isset ($this->authSessionExpire))
            throw new AuthException("Session expire not defined");
        return $this->authSessionExpire;
    }
    public function setPostExpire($postExpire) {
        if (!isset ($postExpire))
            throw new AuthException("Post expire cannot be empty");
        $this->authPostExpire = $postExpire;
    }
    public function getPostExpire() {
        if (!isset ($this->authPostExpire))
            throw new AuthException("Post expire not defined");
        return $this->authPostExpire;
    }
    public function setPostWait($postWait) {
        if (!isset ($postWait))
            throw new AuthException("Post wait cannot be empty");
        $this->authPostWait = $postWait;
    }
    public function getPostWait() {
        if (!isset ($this->authPostWait))
            throw new AuthException("Post wait not defined");
    ...
    public function  __set($name,  $value) {
        if (!isset ($this->appSession->AuthData))
            throw new AuthException("authentication data not initialized");
