...
/**
 * Config contains all the configuration for the system. This class should not be called directly.
 *
 * <p>Framework settings are all in capital letters. Settings can be accessed via <code>Q::conf()->SOME_SETTINGS</code></p>
 *
...
class Config{
    ...
    /**
     * Path to the location of  framework directory. eg. /var/lib/framework/
     * @var string 
     */
    public $BASE_PATH;
    ...
    /**
     * URL of your app. eg. http://localhost/php/
     * @var string 
     */
    public $APP_URL;

    /**
     * Please define SUBFOLDER if your app is not in the root directory of the domain. eg. http://localhost/php , you should set '/php/'
     * @var string 
     */
    public $SUBFOLDER;
     ...
     * Debug tool can be viewed if <code>Q::logger()->showDebugger()</code> is called
     * @var bool 
     */
	 ...
	 * Options are: View and BasicView
     * @var string
	 */
    ...
    /**
     * List of template tags to be used globally in View template engine ( global function names and PHP functions )
     * @var array
     */
     ...
     * <code>
     * //Store
     * Q::conf()->add('latestItem', $item);
     *
     * //Retrieve
     * echo Q::conf()->latestItem;
     * </code>
     *
...
/**
 * WebApp is the global context that processed user's requests.
 *
...
class WebApp{
    ...
    public function routeTo(){
        ...
        if($routeRs[0]!==null && $routeRs[1]!==null){
            ...
            if($routeRs[0][0]!=='['){
                if(strpos($routeRs[0], '\\')!==false){
                    ...
                    $nsClassFile = explode(Q::conf()->APP_NAMESPACE_ID.'/', $nsClassFile, 2);
                    $nsClassFile = $nsClassFile[1];
                    require_once Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER . $nsClassFile .'.php';                    
                }else{
                    require_once Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER . "controller/{$routeRs[0]}.php";
                }
            }else{
                ...
                if(isset(Q::conf()->PROTECTED_FOLDER_ORI)===true){
                    require_once Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER_ORI . 'module/'. $moduleName .'/controller/'.$moduleParts[1].'.php';                    
                }else{
                    require_once Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER . 'module/'. $moduleName .'/controller/'.$moduleParts[1].'.php';                    
                    Q::conf()->PROTECTED_FOLDER_ORI = Q::conf()->PROTECTED_FOLDER;
                }
                
                //set class name
                $routeRs[0] = $moduleParts[1];
                Q::conf()->PROTECTED_FOLDER = Q::conf()->PROTECTED_FOLDER_ORI . 'module/'.$moduleName.'/';
            }
        ...
        //if auto route is on, then auto search Controller->method if route not defined by user
        else if(Q::conf()->AUTOROUTE){

            list($controller_name, $method_name, $method_name_ori, $params, $moduleName )= $router->auto_connect(Q::conf()->SUBFOLDER, (isset($this->route['autoroute_alias'])===true)?$this->route['autoroute_alias']:null );
            ...
            $controller_file = Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER . "controller/{$controller_name}.php";
            ...
            if(file_exists($controller_file)){
                ...
                if($methodsArray===null && isset(Q::conf()->APP_NAMESPACE_ID)===true){
                    if(isset($moduleName)===true){
                        $controller_name = Q::conf()->APP_NAMESPACE_ID . '\\module\\'. $moduleName .'\\controller\\' . $controller_name;                        
                    }else{
                        $controller_name = Q::conf()->APP_NAMESPACE_ID . '\\controller\\' . $controller_name;
                    }
    				$methodsArray = get_class_methods($controller_name);   
                }
                ...
                if($methodsArray===null){
                    if(isset(Q::conf()->PROTECTED_FOLDER_ORI)===true)
                        Q::conf()->PROTECTED_FOLDER = Q::conf()->PROTECTED_FOLDER_ORI;
					$this->throwHeader(404);
					return;                    
                }
            }
            else if(isset($moduleName)===true && isset(Q::conf()->APP_NAMESPACE_ID)===true){
                if(isset(Q::conf()->PROTECTED_FOLDER_ORI)===true)
                    Q::conf()->PROTECTED_FOLDER = Q::conf()->PROTECTED_FOLDER_ORI;                
                
                $controller_file = Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER . '/controller/'.$moduleName.'/'.$controller_name .'.php';                 
                
                if(file_exists($controller_file)===false){
					$this->throwHeader(404);
					return;                    
                }                
                $controller_name = Q::conf()->APP_NAMESPACE_ID .'\\controller\\'.$moduleName.'\\'.$controller_name;                
    ...
    public function reroute($routeuri, $is404=false){
        ...
        if(Q::conf()->SUBFOLDER!='/')
            $_SERVER['REQUEST_URI'] = substr(Q::conf()->SUBFOLDER, 0, strlen(Q::conf()->SUBFOLDER)-1) . $routeuri;
        else
            $_SERVER['REQUEST_URI'] = $routeuri;

        if(isset(Q::conf()->PROTECTED_FOLDER_ORI)===true){
            Q::conf()->PROTECTED_FOLDER = Q::conf()->PROTECTED_FOLDER_ORI;
            unset( Q::conf()->PROTECTED_FOLDER_ORI );
        }
     ...
     * $data['top'] = Q::app()->module('/top/banner');
     * </code>
     *
     * <p>Based on Controller name and Action method:</p>
     * <code>
     * Q::app()->module('MyController', 'renderTop');
     *
     * # If controller is in sub folder
     * Q::app()->module('folder/MyController', 'renderTop');
     *
     * # Passed in parameter if controller is using $this->param['var']
     * Q::app()->module('MyController', 'renderTop', array('nav'=>'banner'));
     * </code>
     *
     * <p>If class name is different from controller filename:</p>
     * <code>
     * # filename is index.php, class name is Admin
     * Q::app()->module(array('index', 'Admin'), 'renderTop');
     *
     * # in a sub folder
     * Q::app()->module(array('admin/index', 'Admin'), 'renderTop');
     *
     * # with parameters
     * Q::app()->module(array('admin/index', 'Admin'), 'renderTop', array('nav'=>'banner'));
     * </code>
     *
    ...
    public function module($moduleUri, $action=null, $params=null){
        if($moduleUri[0]=='/'){
            if(Q::conf()->SUBFOLDER!='/')
                $_SERVER['REQUEST_URI'] = substr(Q::conf()->SUBFOLDER, 0, strlen(Q::conf()->SUBFOLDER)-1) . $moduleUri;
            else
                $_SERVER['REQUEST_URI'] = $moduleUri;

            $tmp = Q::conf()->PROTECTED_FOLDER;
            if(isset(Q::conf()->PROTECTED_FOLDER_ORI)===true){
                Q::conf()->PROTECTED_FOLDER = Q::conf()->PROTECTED_FOLDER_ORI;
                $tmpOri = Q::conf()->PROTECTED_FOLDER_ORI;
            }
            ...
            Q::conf()->PROTECTED_FOLDER = $tmp;
            
            if(isset($tmpOri)===true)
                Q::conf()->PROTECTED_FOLDER_ORI = $tmpOri;
            
            return $data;
        }
        //if Controller name passed in:  Q::app()->module('admin/SomeController', 'login',  array('nav'=>'home'));
            ...
            require_once Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER . "controller/$moduleUri.php";
            $controller = new $controller_name;
            $controller->params = $params;
            ...
            if($controller->autorender===true){
                Q::conf()->AUTO_VIEW_RENDER_PATH = array(strtolower(substr($controller_name, 0, -10)), strtolower(preg_replace('/(?<=\\w)(?=[A-Z])/','-$1', $action)));
            }
        ...
        //eg. Q::app()->module(array('admin/Admin', 'AdminController'), 'login',  array('nav'=>'home'));
        else{
            require_once Q::conf()->SITE_PATH . Q::conf()->PROTECTED_FOLDER . "controller/{$moduleUri[0]}.php";
    ...
    /**
     * Advanced version of WebApp::module(). Process a module from the main application or other modules.
     *
    ...
    public function getModule($moduleName, $moduleUri, $action=null, $params=null){
        if(empty($moduleName)===false){
            if(isset(Q::conf()->PROTECTED_FOLDER_ORI)===false){
                Q::conf()->PROTECTED_FOLDER_ORI = $tmp = Q::conf()->PROTECTED_FOLDER;
                Q::conf()->PROTECTED_FOLDER = $tmp . 'module/'.$moduleName.'/';
        ...
        if($code!=null){
            if(is_int($code)){
                if($code===404){
                    ...
                    header('HTTP/1.1 404 Not Found');
                    if(!empty(Q::conf()->ERROR_404_DOCUMENT)){
                        include Q::conf()->SITE_PATH . Q::conf()->ERROR_404_DOCUMENT;
                    }
                    ...
                    elseif(!empty(Q::conf()->ERROR_404_ROUTE)){
						$this->reroute(Q::conf()->ERROR_404_ROUTE, true);
                    }
                    exit;
                }
                //if not 404, just send the header code
                else{
                    UriRouter::redirect(null,true, $code);
                }
            }
            elseif(is_string($code)){
                //Controller return the redirect location, it sends 302 Found
                UriRouter::redirect($code);
            }
            elseif(is_array($code)){
                ...
                elseif($code[1]===302){
                    UriRouter::redirect($code[0],true, $code[1], array("HTTP/1.1 302 Moved Temporarily"));
                }
                //else redirect with the http status defined,eg. 307
                else{
                    UriRouter::redirect($code[0],true, $code[1]);
                }
            }
        }
    }
    ...
    /**
     * To debug variables with PHP's diagnostic view
 ...
 * in both mod_php or FastCGI mode. Please refer to http://php.com/tutorial/setup to see how to used PHP with Cherokee web server</p>
 *
 ...
 * </p>
 *
 * <p><b>RESTful API</b>s are supported natively. You can mocked up <b>RESTful API</b>s easily with UriRouter.
 ...
 * </code></p>
 *
 * <p>If you need to reverse generate URL based on route ID with UrlBuilder in template view, please defined the id along with the routes
 * <code>
 ...
 * </code></p>
 *
 * <p>If you want a module to be publicly accessed (without using Q::app()->getModule() ) , use [module name] ,   eg. /protected/module/forum/PostController.php
 * <code>
...
class UriRouter{
     ...
     * @param string $subfolder Relative path of the sub directory where the app is located. eg. http://localhost/php, the value should be '/php/'
    ...
    public function execute($routeArr,$subfolder='/'){
        ...
        if(isset($route['auth'])===true){
            ...
            Q::loadCore('auth/DigestAuth');
            DigestAuth::http_auth($route['authName'],$route['auth'], $route['authFail'], $route['authFailURL']);
        }
     ...
     * <p>To use UriRouter without index.php, add the following code to your .htaccess file if Apache mod_rewrite is enabled.</p>
     * <code>
     * RewriteEngine On
     *
	...
	private function connect($routes, $subfolder) {
		...
		if ($skipNormalRoutes===false) {
			...
			if (isset($possibleRoutes[$requestedUri])===true) {
				...
				if (false === strpos($requestedUri, ':')) {
					...
					Q::conf()->AUTO_VIEW_RENDER_PATH = $requestedUri;
		...
		if ($skipNormalRoutes===false) {
			foreach($possibleRoutes as $routeKey=>$routeData) {
				...
				Q::conf()->AUTO_VIEW_RENDER_PATH = $routeKey;
    ...
    public function auto_connect($subfolder='/', $autoroute_alias=null){
        ...
        $module = null;
        if(isset(Q::conf()->MODULES)===true && in_array($uri[0], Q::conf()->MODULES)===true){
            $module = $uri[0];
            array_shift($uri);
        }
		...
		Q::conf()->AUTO_VIEW_RENDER_PATH = array($controller_name);
        ...
        if(empty($uri[1])===false){
			...
			Q::conf()->AUTO_VIEW_RENDER_PATH[] = $uri[1];
		}else{
            $method_name = $method_name_ori = 'index';
			Q::conf()->AUTO_VIEW_RENDER_PATH[] = 'index';
		}
        ...
        if($autoroute_alias!==null){
            ...
            if(isset($autoroute_alias[$alias])===true){
                ...
                Q::conf()->AUTO_VIEW_RENDER_PATH[0] = strtolower(preg_replace('/([A-Z])/', '-$1', substr($convertname, 0, -10)));
            }
            else{
                ...
                if( preg_match('/^('. $aliaskey .')\//', '/'.$uridecode.'/', $matchedKey) > 0){
                    ...
                    Q::conf()->AUTO_VIEW_RENDER_PATH[1] = $method_name;
                }
            }
        }
 ...
 *
 * <p>Provides a few shorthand methods to access commonly used component during development. e.g. Loader, Log, SqlMagic.</p>
 *
 ...
 *
 * <p>GET and POST variables can still be accessed via php $_GET and $_POST. They are not handled/process by  framework.</p>
 *
...
class Controller {
    ...
    /**
     * Returns the database singleton, shorthand to Q::db()
     * @return SqlMagic
     */
    public function db(){
        return Q::db();
    ...
    /**
     * Returns the Acl singleton, shorthand to Q::acl()
     * @return Acl
     */
    public function acl(){
        return Q::acl();
    ...
    /**
     * Returns the cache singleton, shorthand to Q::cache()
     * @return FileCache|FrontCache|ApcCache|MemCache|XCache|EAcceleratorCache
     */
    public function cache($cacheType='file'){
        return Q::cache($cacheType);
    }
     ...
     * @param bool $includeTagClass If true, View will determine which Template tag class to include. Else, no files will be loaded
     * @return string|false The file name of the rendered output saved (html).
     */
     ...
     * @return View|ViewBasic
     */
    public function view(){
        if($this->_view==NULL){
			$engine = Q::conf()->TEMPLATE_ENGINE;
            Q::loadCore('view/' . $engine);
	...
	public function viewRenderAutomation(){
		if(is_string(Q::conf()->AUTO_VIEW_RENDER_PATH)){
			$path = Q::conf()->AUTO_VIEW_RENDER_PATH;
			$path = str_replace(':', '@', substr($path, 1));
			$this->{$this->renderMethod}($path, $this->vdata);
		}else{
            if(isset(Q::conf()->AUTO_VIEW_RENDER_PATH))
                $this->{$this->renderMethod}(strtolower(Q::conf()->AUTO_VIEW_RENDER_PATH[0]) .'/'. strtolower(Q::conf()->AUTO_VIEW_RENDER_PATH[1]), $this->vdata);
            else
	...
	public function  __call($name,  $arguments) {
		if ($name == 'renderLayout') {
			throw new Exception('renderLayout is no longer supported by Controller. Please use $this->view()->renderLayout instead');
		}
	}
